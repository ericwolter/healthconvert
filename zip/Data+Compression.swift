...
extension Data {
    ...
    static func process(operation: compression_stream_operation, size: Int, bufferSize: Int,
        ...
        repeat {
            ...
            print("1", position, stream.src_size, stream.dst_size)
            position = try self.fill(stream: &stream, checksum: &checksum, chunk: &chunk,
                                     ...
                                     provider: provider)
            print("2", position, stream.src_size, stream.dst_size)
            ...
            print("3", position, stream.src_size, stream.dst_size)
            if position >= size { flags = Int32(COMPRESSION_STREAM_FINALIZE.rawValue) }
            print("4", position, stream.src_size, stream.dst_size)
            status = compression_stream_process(&stream, flags)
            print("5", position, stream.src_size, stream.dst_size)
            switch status {
            ...
            print("6", position, stream.src_size, stream.dst_size)

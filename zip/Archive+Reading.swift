...
extension Archive {
  ...
  public func extractFile(_ entry: Entry, bufferSize: UInt32 = defaultReadChunkSize) throws -> ZipInputStream {
    fseek(self.archiveFile, entry.dataOffset, SEEK_SET)

    let stream = ZipInputStream(archive: self.archiveFile, entry: entry)
    return stream
